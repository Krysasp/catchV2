...
class ProbeDesigner:
    ...
    def _cluster_genomes(self):
            ...
            for g in genomes_from_group:
                ...
                for s in g_seqs:
                    if (self.seq_length_to_skip is not None and
                        ...
                        continue
                    seqs[seq_idx] = s
                    seq_headers[seq_idx] = getattr(g, 'header', None)
                    seq_idx += 1
        ...
        for clust in clusters:
            ...
            for seq_idx in clust:
                seq = seqs[seq_idx]
                header = seq_headers.get(seq_idx)
                genomes_in_clust += [genome.Genome.from_one_seq(seq, header=header)]
    ...
    def _design_for_genomes(self, genomes, filters):
        ...
        for genomes_from_group in genomes:
            ...
            for g in genomes_from_group:
                source_headers = [getattr(g, 'header', None)] * len(g.seqs)
