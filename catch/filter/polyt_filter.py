...
class PolyTFilter(BaseFilter):
    """Filter that removes probes with poly(T).
    """
    ...
    def __init__(self, length, mismatches, min_exact_length_to_consider=6):
        ...
        Args:
            length/mismatches: remove probes that contain at least
                LENGTH 'T' bases in a row, tolerating up to MISMATCHES
                mismatches
            min_exact_length_to_consider: only look for a stretch of 'T'
                (according to length/mismatches) in a probe if that
                probe contains an exact stretch of 'T' that is
    ...
    def _filter(self, input):
        ...
        for p in input:
            keep = True
            if exact_t_stretch in p.seq_str:
                lcf_len = p.longest_common_substring_length(
                    t_stretch, self.mismatches)
