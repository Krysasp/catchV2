...
def make_candidate_probes_from_sequence(seq,
    ...
    Args:
        ...
        source_header: the FASTA header from which this sequence was derived
        ...
        if allow_small_seqs:
            if len(seq) < allow_small_seqs:
                ...
                if n_string_query.search(seq):
                    ...
                    return [probe.Probe.from_str(seq, source_header=source_header)]
        else:
    ...
    def add_probe_from_subsequence(start, end,
        ...
        # list of probe.Probe
        probes = [probe.Probe.from_str(p, source_header=source_header) for p in probes]
        for p in probes:
...
def make_candidate_probes_from_sequences(
    ...
    Args:
        ...
        seq_length_to_skip: if set, skip sequences whose length is <=
            ...
            them)
        source_headers: optional list of FASTA headers corresponding to each
            sequence in seqs; if provided, each probe will be tagged with
            its source header
    ...
    for idx, seq in enumerate(seqs):
        ...
        source_header = source_headers[idx] if source_headers else None
