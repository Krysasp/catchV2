...
class PolyCFilter(BaseFilter):
    """Filter that removes probes with poly(C).
    """
    ...
    def __init__(self, length, mismatches, min_exact_length_to_consider=6):
        ...
        Args:
            length/mismatches: remove probes that contain at least
                LENGTH 'C' bases in a row, tolerating up to MISMATCHES
                mismatches
            min_exact_length_to_consider: only look for a stretch of 'C'
                (according to length/mismatches) in a probe if that
                probe contains an exact stretch of 'C' that is
    ...
    def _filter(self, input):
        ...
        exact_c_stretch = 'C'*self.min_exact_length_to_consider

        probe_len = len(input[0])
        for p in input:
            probe_len = max(probe_len, len(p))
        c_stretch = probe.Probe.from_str('C'*probe_len)

        out = []
        for p in input:
            keep = True
            if exact_c_stretch in p.seq_str:
                lcf_len = p.longest_common_substring_length(
                    c_stretch, self.mismatches)
