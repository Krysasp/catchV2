...
class Probe:
    ...
    def __init__(self, seq, source_header=None):
        ...
        Args:
            ...
            source_header: the FASTA header from which this probe was derived
        """
        self.seq = seq
    ...
    def identifier(self, length=10):
        ...
        input yield larger hashes); using the SHA-224 digest function
        ...
        If a source_header is set, the identifier will be prefixed with
        the source_header prefix (e.g., 'e2b35f2c5a_CONS|5' or 
        'e2b35f2c5a_CONS|5|72.5|66.7' when Tm values are included).

        Returns:
            a (probably) unique identifier for this probe, as a string
        """
        base_id = hashlib.sha224(self.seq_str.encode()).hexdigest()[-length:]
        if self.source_header:
            # Extract prefix from source_header
            # Format: 'prefix|template_tm|oligo_tm' or 'prefix|subsuffix'
            parts = self.source_header.split('|')
            if len(parts) >= 4:
                # Tm values are included (e.g., 'CONS|5|72.5|66.7')
                prefix = parts[0] + '|' + parts[1] + '|' + parts[2] + '|' + parts[3]
            elif len(parts) >= 3:
                # Partial Tm info (e.g., 'CONS|5|72.5')
                prefix = parts[0] + '|' + parts[1] + '|' + parts[2]
            elif len(parts) >= 2:
                # Just prefix (e.g., 'CONS|5')
                prefix = parts[0] + '|' + parts[1]
            else:
                prefix = parts[0]
            return f"{base_id}_{prefix}"
        return base_id
    ...
    @staticmethod
    def from_str(seq_str, source_header=None):
        ...
        """
        return Probe(np.fromiter(seq_str, dtype='U1'), source_header=source_header)
