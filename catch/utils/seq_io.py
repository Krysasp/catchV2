...
def read_genomes_from_fasta(fn):
    ...
    that genomes are not divided into multiple chromosomes within
    the fasta.
    ...
    genomes = []
    fasta_dict = read_fasta(fn)
    for header, seq in fasta_dict.items():
        genomes += [genome.Genome.from_one_seq(seq, header=header)]
    return genomes
...
def write_probe_fasta(probes, out_fn, use_header_if_set=True):
    ...
    Args:
        ...
        use_header_if_set: if True, use p.header if set; otherwise always
            use p.identifier() (which includes source_header prefix if available)
    """
    with open(out_fn, 'w') as f:
        for p in probes:
            if use_header_if_set and p.header:
                f.write('>' + p.header + '\n')
            else:
                f.write('>' + p.identifier() + '\n')
            f.write(p.seq_str + '\n')
